...
EnableLog('email_report_hyper_aviapark');

function log(message) {
    LogEvent('email_report_hyper_aviapark', message);
    // alert(message);
}
...
function setArrayTeElems(elemIds) {
    resultArray = [];
    // log(tools.object_to_text(elemIds, "json"));
...
function sendReport(files, personId, title, textBody) {
    try
    {
        ...
        log('message body : ' + tools.object_to_text( newNotify, 'json'));
...
try
{
    ...
    try
    {
        ...
        log("setArrayTeElems for courses");
        _coursesTopElems = setArrayTeElems(_coursesIds);
        log("setArrayTeElems for events");
        _eventsTopElems = setArrayTeElems(_eventsIds);
        log("setArrayTeElems for tests");
    ...
    {
        log('Создать заголовок для таблицы ' + String(ex));
    }
    ...
    for(_groupId in _groupsIds) {
        ...
        try {
            ...
            for(_teTest in _testsTopElems) {
                ...
                // _trainingResultMessage += "<div>Процент обученности по тесту '" + _teTest.name + "': " + _percentage + "%<div>"; // АКТУАЛЬНАЯ
                // _trainingResultMessage += "<div>Проверка на ошибку Math.round(_percentage)<div>";
            }
