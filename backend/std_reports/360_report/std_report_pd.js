...
function columnsInit(columns, competencesElems, cenElem, kpisTasks) {
    ...
    addColumn(columns, "ФИО оцениваемого", "ListElem.selfFullname", "string");
    addColumn(columns, "ФИО оценивающего руководителя", "ListElem.managerFullname", "string");
    ...
    addColumn(columns, "Текущий этап", "ListElem.currentStage", "string");
    addColumn(columns, "Итоговая оценка (Цифра)", "ListElem.overall_number", "string");
    addColumn(columns, "Итоговая оценка (Буква)", "ListElem.overall_letter", "string");

    setKpisColumns(columns, kpisTasks);

    setCenColumns(columns, cenElem, "self");
    addColumn(columns, "Итоговый балл - ценности (Самооценка)", "ListElem.total_mark_cen_self", "string");
    setCompetencesColumns(columns, competencesElems, "self");
    addColumn(columns, "Итоговый балл - компетенции (Самооценка)", "ListElem.total_mark_self", "string");

    setCenColumns(columns, cenElem, "manager");
    addColumn(columns, "Итоговый балл - ценности (Оценка руководителем)", "ListElem.total_mark_cen_manager", "string");
    setCompetencesColumns(columns, competencesElems, "manager");
    addColumn(columns, "Итоговый балл - компетеции (Оценка руководителем)", "ListElem.total_mark_manager", "string");
}


function setKpisColumns(columns, kpisTasks) {

    addColumn(columns, "Бюджетный период", "ListElem.budget_period", "string");
    addColumn(columns, "Показатели", "ListElem.kpi_tasks", "string");
    ...
    addColumn(columns, "KPI: Оценка руководителя", "ListElem.kpi_result_mark", "string");
}
...
function setCenColumns(columns, cenElem, prefix) {
    ...
    for(j = 0; j < ArrayCount(cenElem.indicators); j++) {
        ...
        addColumn(columns, _indicator.name + ": ("+statusPa+")" , "ListElem."+ prefix +"cenIndicatorScore_" + cenElem.indicators[j].indicator_id, "string");
        indicatorCounter++;
    }
}
...
function setCompetencesColumns(columns, competencesElems, prefix) {
    ...
    for(i = 0; i < ArrayCount(competencesElems); i++) {
        ...
        addColumn(columns, _competence.name + " - общий балл ("+statusPa+"): ", "ListElem."+ prefix +"CompetenceScore_" + competencesElems[i].competence_id, "string");
        competenceCounter++;      
        ...
        for(j = 0; j < ArrayCount(competencesElems[i].indicators); j++) {
            ...
            addColumn(columns, _indicator.name + ": (Самооценка)" , "ListElem."+ prefix +"indicatorScore_" + competencesElems[i].indicators[j].indicator_id, "string");
            indicatorCounter++;
        }
    }
}
...
for(_assessmentPlan in _assessmentPlans) {
    ...
    try {
        ...
        if( !_columnsInit ) {
            columnsInit(columns, _competences, _cen, _teKpisPa.tasks);
            _columnsInit = true;
        }
